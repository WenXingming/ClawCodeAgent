...
- 自动化脚本只覆盖**环境无关**的检查：full tests、orchestration regression、CLI `--help` smoke、release docs validation；需要真实模型后端的交互式演示保留在 `docs/release/DEMO_SCRIPT.md` 中手工执行。
- 发布门禁文档拆分为三个职责清晰的文件：`docs/release/TEST_MATRIX.md` 负责覆盖面矩阵，`docs/release/RELEASE_GATE_CHECKLIST.md` 负责发布流程与结果模板，`docs/release/DEMO_SCRIPT.md` 负责人工演示脚本。
