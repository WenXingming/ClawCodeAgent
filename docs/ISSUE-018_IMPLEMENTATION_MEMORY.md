...
| 文件 | 变更类型 | 说明 |
|------|----------|------|
| `src/runtime/plan_runtime.py` | 新建 | 实现 PlanStep、计划存储、渲染与 plan-task 同步 |
| `src/runtime/task_runtime.py` | 修改 | 新增 `replace_tasks()`，支持计划同步后的整表替换 |
| `test/runtime/test_plan_runtime.py` | 新建 | 覆盖更新同步、依赖映射、状态回写与清空同步 |
...
ISSUE-018 的目标是“计划更新、清空及与 task 同步”，不是让代理立即消费计划。因此当前实现把 `runtime/plan_runtime.py` 设计成与 `agent_runtime.py` 解耦的独立模块，后续再由 control plane 或 workflow issue 接入。
...
- `python -m unittest discover -s test/runtime -p "test_plan_runtime.py" -v` → 4/4 OK