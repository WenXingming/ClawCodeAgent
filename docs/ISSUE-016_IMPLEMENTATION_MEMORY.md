...
| 文件 | 变更类型 | 说明 |
|------|----------|------|
| `src/runtime/plugin_runtime.py` | 修改 | 插件 manifest 新增 hook/block 字段解析，并暴露 tool pipeline helper |
| `src/runtime/hook_policy_runtime.py` | 修改 | policy runtime 新增 execution-time block 决策与 hook helper |
| `src/runtime/agent_runtime.py` | 修改 | 在 tool loop 中接入 preflight、block、after-hook 与审计事件 |
| `src/session/session_state.py` | 修改 | 新增 runtime system message 追加接口，保证 transcript 与 messages 同步 |
| `test/runtime/test_agent_runtime.py` | 修改 | 增加插件阻断、策略阻断、双重注入三条主循环集成测试 |
| `test/runtime/test_plugin_runtime.py` | 修改 | 增加 plugin helper 单元测试 |
| `test/runtime/test_hook_policy_runtime.py` | 修改 | 增加 policy helper 单元测试 |
| `README.md` | 修改 | 更新 plugin/policy manifest 的 hook/block 能力说明 |
| `docs/Architecture.md` | 修改 | 写回 tool pipeline 中 plugin/policy 接入点 |
...
| 测试文件 | 测试方法/分组 | 验证点 |
|----------|---------------|--------|
| `test_agent_runtime` | plugin block / policy block / double inject（3 个） | 插件阻断、策略阻断、plugin+policy 双重注入顺序、tool metadata、runtime events |
| `test_plugin_runtime` | hook/block helper（1 个） | 插件 manifest 的 deny_prefixes / before_hooks / after_hooks 被 helper 正确暴露 |
| `test_hook_policy_runtime` | hook/block helper（1 个） | policy runtime 的 merged fallback 与 block helper 正常工作 |

## 回归结果

定向验证：

- `python -m unittest discover -s test/runtime -p "test_agent_runtime.py" -v` → 28/28 OK
- `python -m unittest discover -s test/runtime -p "test_*_runtime.py" -v` → 34/34 OK
