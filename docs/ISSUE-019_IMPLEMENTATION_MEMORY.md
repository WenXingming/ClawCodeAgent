...
| 文件 | 变更类型 | 说明 |
|------|----------|------|
| `src/runtime/workflow_runtime.py` | 新建 | 实现 workflow manifest 发现、顺序执行与运行历史持久化 |
| `test/runtime/test_workflow_runtime.py` | 新建 | 覆盖发现、运行成功、运行失败与历史回放 |
...
ISSUE-019 的目标是“工作流定义读取、运行和历史记录”，不是马上让 agent 消费 workflow。因此当前实现把 `runtime/workflow_runtime.py` 保持为独立模块，后续再由 control plane 或 workflow 相关 issue 接入。
...
| 测试文件 | 测试方法/分组 | 验证点 |
|----------|---------------|--------|
| `test_workflow_runtime` | discovery（1 个） | workflow manifest 可被发现、列出和读取 |
| `test_workflow_runtime` | run success（1 个） | 顺序执行 task 动作成功，任务状态更新并写入历史文件 |
| `test_workflow_runtime` | run failure（1 个） | 非法状态迁移会被记录为 failed run，错误信息可诊断且历史可重载 |

## 回归结果

定向验证：

- `python -m unittest discover -s test/runtime -p "test_workflow_runtime.py" -v` → 3/3 OK