...
根据 [docs/GLOBAL_REFACTOR_PLAN.md](docs/GLOBAL_REFACTOR_PLAN.md) 的 Step 5，本步目标是把插件、策略、搜索与 worktree 统一收口到新的 `workspace` 领域中，引入 `WorkspaceGateway` 作为唯一工作区门面，并移除 `LocalAgent` 对旧 `extensions` runtime 细节的直接认知。
