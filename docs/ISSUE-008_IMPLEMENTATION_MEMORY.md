...
| 测试文件 | 测试方法 | 验证点 |
|----------|----------|--------|
| test_session_state | test_from_persisted_restores_messages_and_count | 消息列表与 tool_call_count 完整恢复 |
| test_session_state | test_from_persisted_empty_transcript_falls_back_to_messages | transcript 为空时 fallback |
| test_session_state | test_from_persisted_preserves_nonempty_transcript | 已有 transcript 不被覆盖 |
| test_session_state | test_from_persisted_then_append_user_extends_state | 恢复后可继续追加消息 |
| test_agent_runtime | test_resume_session_id_does_not_drift | session_id 不漂移 |
| test_agent_runtime | test_resume_accumulates_usage_turns_and_tool_calls | 预算累计正确 |
| test_agent_runtime | test_resume_model_sees_history_context | 上下文连续可见 |
| test_agent_runtime | test_resume_backend_error_preserves_session_id_and_saves | 错误时 session_id 保持 |
| test_main | test_main_session_id_triggers_resume_not_run | --session-id 走 resume 路径 |
| test_main | test_main_resume_missing_session_returns_error | 不存在时 exit=2 + stderr 输出 |
| test_main | test_main_without_session_id_still_runs_normally | 无 --session-id 无回归 |
