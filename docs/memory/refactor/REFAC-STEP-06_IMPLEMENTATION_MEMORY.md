...
根据 [docs/architecture/GLOBAL_REFACTOR_PLAN.md](/docs/architecture/GLOBAL_REFACTOR_PLAN.md) 的 Step 6，本步目标是把 context 领域重建为单一门面结构：引入 `ContextManager`，将旧的预算投影、snip、compact 与 pre-model 编排职责重组为新的 context 子模块，并删除旧 `BudgetContextOrchestrator` 这条跨层历史边界。
...
11. 更新 [docs/architecture/Architecture.md](/docs/architecture/Architecture.md)，使当前活文档与新的 context 门面结构、测试拓扑和当前 interaction/tools 目录一致。
