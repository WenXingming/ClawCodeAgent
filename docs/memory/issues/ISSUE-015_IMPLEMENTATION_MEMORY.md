...
| `src/tools/local_tools.py` | 修改 | `ToolExecutionContext` 新增 `safe_env`，bash 子进程显式透传环境变量 |
...
| `test/tools/test_local_tools_shell.py` | 修改 | 增加 safe env 传入 bash 子进程的测试 |
| `README.md` | 修改 | 补充工作区 policy manifest 发现路径、字段示例与合并规则 |
| `docs/architecture/Architecture.md` | 修改 | 写回 hook policy runtime 在当前架构中的位置 |
...
| `test_local_tools_shell` | `test_bash_passes_safe_env_to_subprocess` | safe env 确实进入 bash 子进程 `env` |
...
- `python -m unittest discover -s test/tools -p "test_local_tools_shell.py" -v` → 7/7 OK
