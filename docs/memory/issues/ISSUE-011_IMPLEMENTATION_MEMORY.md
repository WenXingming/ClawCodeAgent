...
| `docs/architecture/Architecture.md` | 修改 | ContextPkg 追加 `context_compactor.py` 节点与依赖 |
| `docs/architecture/FINAL_ARCHITECTURE_PLAN.md` | 追加 | ISSUE-011 实施决策归档 |
...
`ContextCompactor.is_context_length_error()` 仅在后端响应语义明确属于 prompt-too-long /
context window exceeded 时返回 True。普通连接错误、超时或通用 backend_error
不进入 compact 恢复路径，保持原有失败语义。

### 4. compact 摘要调用禁用工具，输出纯文本摘要
`ContextCompactor.compact()` 调用模型时固定传 `tools=[]`，要求返回纯文本摘要，
