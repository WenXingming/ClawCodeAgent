...
| 文件 | 变更类型 | 说明 |
|------|----------|------|
| `src/interaction/slash_commands_interaction.py` | 修改 | 重构为 `SlashCommandDispatcher` 分发器类，保留兼容包装函数，并补齐文件级到函数级注释 |
| `src/orchestration/agent_runtime.py` | 修改 | 在 prompt 写入 session 前接入 slash 预分流与本地结果落盘 |
| `test/interaction/test_slash_commands.py` | 新建 | slash 解析与命令分发单元测试 |
| `test/orchestration/test_agent_runtime.py` | 追加 | `/help`、`/status`、`/clear` 的 no-model-call 集成测试 |
| `docs/architecture/Architecture.md` | 修改 | 主图补充 `interaction/slash_commands_interaction.py` 控制面节点 |
| `docs/architecture/FINAL_ARCHITECTURE_PLAN.md` | 修改 | ISSUE-012 章节写回实际落地决策 |
...
`src/interaction/slash_commands_interaction.py` 不直接依赖 `LocalCodingAgent`，而是通过 `SlashCommandContext` 读取会话、配置、模型和工具注册表。这样避免了 runtime 与 slash 模块的循环依赖，也方便单测直接构造上下文。

### 5. 分发逻辑收敛为面向对象分发器，但保留兼容入口
在不改动 runtime 调用方式的前提下，模块内部已重构为 `SlashCommandDispatcher`。状态性依赖（如 `ContextTokenBudgetEvaluator`、命令规格索引）集中在对象内部管理，模块级 `parse_slash_command()` / `dispatch_slash_command()` / `get_slash_command_specs()` / `find_slash_command()` 仅作为默认分发器的兼容包装层。

### 6. `/context` 复用既有 token 预算投影
本期没有新增独立的上下文估算逻辑，而是直接复用 `ContextTokenBudgetEvaluator.evaluate()` 与工具 schema 投影，让控制面看到的 projected tokens 与 runtime 预算检查语义保持一致。

## 测试覆盖（新增 +10）

| 测试文件 | 测试方法/分组 | 验证点 |
|----------|---------------|--------|
| `test/interaction/test_slash_commands.py` | 解析测试（2 个） | slash 命令提取、普通文本透传 |
| `test/interaction/test_slash_commands.py` | 分发测试（4 个） | 未知命令、`/context`、`/tools`、`/clear` 的本地结果语义 |
| `test/interaction/test_slash_commands.py` | 分发器公有 API（1 个） | 直接验证 `SlashCommandDispatcher.parse/find` 的 OO 入口行为 |
...
- `$env:PYTHONPATH='src'; python -m unittest discover -s test/interaction -p "test_slash_commands.py" -v` → 8/8 OK
