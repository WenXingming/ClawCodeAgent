...
1. 定义 `StoredAgentSession` 落盘结构。
2. 提供 `save_agent_session(...)` / `load_agent_session(...)`。
...
3. `load_agent_session(...)` 只支持 “`session_id + 可选 directory`” 这一种基础入口。
...
4. 配置对象使用强类型恢复：
   - `StoredAgentSession` 内部直接持有 `ModelConfig` / `AgentRuntimeConfig` / `TokenUsage`，方便 ISSUE-008 直接接续。
...
2. 能从文件恢复基础消息和配置：✅
   ...
   - 证据：`StoredAgentSession.from_dict(...)` 直接恢复强类型配置对象
3. usage/cost 字段不丢失：✅
...
1. `load_agent_session(...)` 已经能恢复配置对象、messages、usage 和 stop reason，ISSUE-008 可直接在此基础上恢复 `AgentSessionState`。
2. 当前 `session_id` 已在整个 `run()` 生命周期内稳定，ISSUE-008 可沿用同一 `session_id` 继续保存，避免 session 漂移。
3. 若后续需要预算继承，优先从 `StoredAgentSession.usage / total_cost_usd / turns / tool_calls` 建立基线，而不是重新扫描 transcript。
