...
根据 [docs/GLOBAL_REFACTOR_PLAN.md](docs/GLOBAL_REFACTOR_PLAN.md) 的 Step 3，引入单一动态运行态对象 `AgentRunState`，把 turn、usage、events、budget snapshot、tool count、MCP capability window 与当前 turn 的有效工具表从 `LocalAgent` 的局部变量和 `AgentSessionState` 中收口出来。
