...
| 文件 | 变更类型 | 说明 |
|------|----------|------|
| `src/interface/__init__.py` | 新建 | 建立 interface 包边界 |
| `src/interface/slash_commands_interface.py` | 新建 | 下沉 slash 命令实现为控制面子模块 |
| `src/interface/command_line_interface.py` | 新建/扩展 | 子命令 parser、配置覆盖、chat loop 与命令装配 |
| `src/main.py` | 修改 | 收敛为极薄的 CLI 进程入口包装层 |
| `src/orchestration/agent_runtime.py` | 修改 | slash 导入切换到 interface 包 |
| `test/test_main.py` | 修改 | 迁移到 `agent` / `agent-resume` 子命令并覆盖配置覆盖逻辑 |
| `test/test_main_chat.py` | 新建 | `agent-chat` 交互循环与 `/clear` 会话切换测试 |
| `test/interface/test_slash_commands.py` | 修改 | 测试导入切换到 `interface.slash_commands_interface` |
| `docs/Architecture.md` | 修改 | 架构图更新为 `interface/command_line_interface.py + interface/slash_commands_interface.py` |
...
ISSUE-013 后，CLI 不再只是一个最小脚本，而是带子命令、覆盖逻辑与 chat loop 的命令面。把 CLI 和 slash 一起下沉到 `src/interface/` 可以把“控制面复杂度”从源码根移走，同时保留 `src/main.py` 作为稳定进程入口与测试 patch 点。
...
| `test/interface/test_slash_commands.py` | 模块导入迁移 | slash 测试转到 `interface.slash_commands_interface` |
...
- `python -m unittest discover -s test/interface -p "test_slash_commands.py" -v`（interface 包化后）→ OK
