...
| `src/agent_slash_commands.py` | 修改 | 退化为兼容 shim，转发到 `control_plane.slash_commands` |
| `src/runtime/agent_runtime.py` | 修改 | slash 导入切换到 control_plane 包 |
| `test/runtime/test_main.py` | 修改 | 迁移到 `agent` / `agent-resume` 子命令并覆盖配置覆盖逻辑 |
| `test/runtime/test_main_chat.py` | 新建 | `agent-chat` 交互循环与 `/clear` 会话切换测试 |
| `test/runtime/test_agent_slash_commands.py` | 修改 | 测试导入切换到 `control_plane.slash_commands` |
...
| 测试文件 | 测试方法/分组 | 验证点 |
|----------|---------------|--------|
| `test_main` | 子命令入口（4 个） | 必需子命令、拒绝旧裸 prompt、`agent` 基本执行、环境变量回退 |
| `test_main` | `agent-resume`（3 个） | resume 路径、缺失 session 错误、存档配置覆盖 |
| `test_main` | 权限验证（2 个） | `allow_destructive_shell` 依赖 `allow_shell`，显式权限映射正确 |
| `test_main_chat` | chat 基础行为（4 个） | 初始 prompt、resume 续聊、`.quit` 退出、EOF 退出 |
| `test_main_chat` | `/clear` 会话切换（1 个） | chat loop 在 slash fork 后切换到新的 session_id |
| `test_agent_slash_commands` | 模块导入迁移 | slash 测试转到 `control_plane.slash_commands` |

## 回归结果

定向验证：

- `python -m unittest test_main test_main_chat -v` → 15/15 OK
- `python -m unittest test_agent_slash_commands test_main -v`（control_plane 包化后）→ OK
