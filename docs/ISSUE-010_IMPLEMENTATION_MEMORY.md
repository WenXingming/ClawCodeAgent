...
`snip_session()` 先通过 `_count_prefix()` 跳过头部连续 `system` 消息，
...
`snip_session(messages: list[dict])` 不依赖 `AgentSessionState`，而是直接操作
...
| `test_snip` | `snip_session` 中间段剪裁（7 个） | 空消息、无候选、middle snip、tail 保留、prefix 保留、重复 tombstone 跳过、tokens_removed 非负 |
