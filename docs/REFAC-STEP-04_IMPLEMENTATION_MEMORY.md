...
根据 [docs/GLOBAL_REFACTOR_PLAN.md](docs/GLOBAL_REFACTOR_PLAN.md) 的 Step 4，本步目标是把工具执行重建为独立子系统，建立统一门面，清理 `LocalAgent` 对工具上下文、工具 schema 构建、bash 流式输出和 MCP 文件布局的直接认知。
