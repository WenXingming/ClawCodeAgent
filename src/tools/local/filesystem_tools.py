...
所有工具通过 FileSystemToolProvider.build_tools 工厂方法产出 ToolDescriptor 注册表条目。
"""
...
class FileSystemToolProvider:
    ...
    将 list_dir、read_file、write_file、edit_file 四个工具的定义与处理逻辑
    封装为高内聚的面向对象结构，对外仅暴露 build_tools 工厂方法。
    """
    ...
    # ── 私有方法（按 build_tools 中首次引用顺序深度优先排列） ──────

    def _list_dir(self, arguments: JSONDict, context: ToolExecutionContext) -> str | tuple[str, JSONDict]:
        """列出工作区内目录内容。"""
        raw_path = self._get_string(arguments, 'path', default='.')
        max_entries = self._get_int(arguments, 'max_entries', default=200, min_value=1, max_value=500)
        ...
        rel = self._to_relative_display(target, context.root)
        ...
        content = '\n'.join(lines)
        output = self._truncate_output(content, context.max_output_chars)
        return (
            output,
            {
                'action': 'list_dir',
    ...
    def _read_file(self, arguments: JSONDict, context: ToolExecutionContext) -> str | tuple[str, JSONDict]:
        """读取工作区内文本文件，可选按行裁剪。"""
        raw_path = self._require_string(arguments, 'path')
        line_slice = self._parse_line_slice(arguments)
        ...
        sliced_text = self._slice_text_by_line(text, line_slice)

        output = self._truncate_output(sliced_text, context.max_output_chars)
        rel = self._to_relative_display(target, context.root)
        return (
            output,
            {
    ...
    def _write_file(self, arguments: JSONDict, context: ToolExecutionContext) -> str | tuple[str, JSONDict]:
        """写入或创建工作区内文件。"""
        self._ensure_write_allowed(context)

        raw_path = self._require_string(arguments, 'path')
        content = self._require_string(arguments, 'content')
    ...
    def _edit_file(self, arguments: JSONDict, context: ToolExecutionContext) -> str | tuple[str, JSONDict]:
        """在工作区文件内执行精确文本替换。"""
        self._ensure_write_allowed(context)

        request = self._parse_edit_request(arguments)
        target = self._resolve_workspace_path(
            context=context,
    ...
    def _parse_line_slice(self, arguments: JSONDict) -> _TextSlice:
        """把 start_line 与 end_line 参数归一化为行切片对象。"""
        start_line = self._get_optional_int(arguments, 'start_line', min_value=1)
        end_line = self._get_optional_int(arguments, 'end_line', min_value=1)
    ...
    def _slice_text_by_line(self, text: str, line_slice: _TextSlice) -> str:
        """按 1-based 闭区间切片截取文本。"""
    ...
    def _ensure_write_allowed(self, context: ToolExecutionContext) -> None:
        """检查当前上下文是否允许写文件。"""
        if context.permissions.allow_file_write:
            return
        raise ToolPermissionError('File write permission denied: allow_file_write=false')

    def _parse_edit_request(self, arguments: JSONDict) -> _FileEditRequest:
        ...
        return _FileEditRequest(
            ...
            replace_all=self._get_bool(arguments, 'replace_all', default=False),
        )
