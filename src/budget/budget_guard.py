...
class BudgetGuard:
    ...
    def check_pre_model(
        ...
        *,
        ...
        Args:
            ...
            snapshot (ContextTokenBudgetSnapshot): 本次模型调用前生成的上下文预算快照。
    ...
    def _check_token(self, snapshot: ContextTokenBudgetSnapshot) -> str | None:
        """内部方法：检查是否超过token硬限制。

        Args:
            snapshot (ContextTokenBudgetSnapshot): 本次模型调用对应的预算快照。
        Returns:
