...
class ContextCompactor:
    ...
    典型工作流如下：
    1. runtime 在显式阈值命中时调用 `compact()` 执行主动压缩。
    2. 若模型返回 context-length 类错误，runtime 会通过 `is_context_length_error()`
       判定是否进入 reactive compact 重试。
    3. 压缩前后统一复用 `ContextTokenEstimator` 记录 token 变化，保证观测口径一致。
    """

    client: OpenAIClient  # OpenAIClient：用于发起摘要压缩模型调用的客户端。
    token_estimator: ContextTokenEstimator = field(default_factory=ContextTokenEstimator)  # ContextTokenEstimator：用于记录压缩前后的 token 变化。
    ...
    def compact(
        ...
        *,
        ...
        Args:
            ...
            preserve_messages (int): 尾部保留不参与压缩的最近消息数量。

        Returns:
            CompactionResult: 本次 compact 的结果统计；失败时 `compacted=False` 并携带错误信息。
        """
    ...
    def should_auto_compact(
        ...
        Args:
            ...
            auto_compact_threshold_tokens (int | None): 配置中的 auto compact 阈值；None 表示关闭。

        Returns:
            bool: 达到或超过阈值时返回 True，否则返回 False。
        """
    ...
    def _build_request_messages(
        ...
        *,
        ...
        Args:
            ...
            preserve_messages (int): 尾部保留不参与压缩的消息数量。

        Returns:
            list[JSONDict] | None: 可直接发送给模型的请求消息；若不存在可压缩区间则返回 None。
        """
    ...
    def _format_summary(self, summary: str) -> str:
        ...
        Returns:
            str: 去除首尾空白并折叠连续空行后的摘要文本；空摘要返回空字符串。
        """
        text = summary.strip()
        if not text:
            return ''
    ...
    def _apply_summary(
        ...
        *,
        ...
        Returns:
            CompactionResult: 本次消息替换的统计结果；若未发生替换则返回错误说明。
        """
        formatted_summary = self._format_summary(summary_text)
        if not formatted_summary:
            return CompactionResult(compacted=False, error='Compact summary is empty')
        ...
        post_tokens = self.token_estimator.estimate_messages(new_messages)
